...
2. Enter the following information for the rental scenario based on {{STATE_NAME}} market conditions:
   - Monthly rent: ${{STATE_MEDIAN_RENT}} (median rent in {{STATE_NAME}})
   - Expected annual rent increase: 3%
   - Renter's insurance: $15/month
   - Utilities not included in rent: $150/month

3. Enter the following information for the buying scenario based on {{STATE_NAME}} market conditions:
   - Home purchase price: ${{STATE_MEDIAN_HOME_PRICE}} (median home price in {{STATE_NAME}})
   - Down payment percentage: 10%
   - Mortgage interest rate: {{STATE_AVG_MORTGAGE_RATE_30YR}}%
   - Loan term: 30 years
   - Property tax rate: {{STATE_PROPERTY_TAX_COUNTY_RATE}}% annually
   - Homeowner's insurance: ${{STATE_INSURANCE_HOMEOWNERS_AVG}}/month
   - HOA fees: $100/month
   - Expected maintenance costs: 1% of home value annually
   - Expected home appreciation rate: Based on {{STATE_HOUSING_MARKET_TRENDS}}
...
1. Select a city or area where you might live in the future (or analyze your current location in {{STATE_NAME}})

2. Research and document:
   - Average rent for apartments/homes that would meet your needs
     * Starting point for {{STATE_NAME}}: Median rent is ${{STATE_MEDIAN_RENT}}
   - Average home prices for properties that would meet your needs
     * Starting point for {{STATE_NAME}}: Median home price is ${{STATE_MEDIAN_HOME_PRICE}}
   - Typical property taxes in the area
     * {{STATE_NAME}} average property tax rate: {{STATE_PROPERTY_TAX_COUNTY_RATE}}%
   - Insurance costs for both renters and homeowners
     * {{STATE_NAME}} average homeowner's insurance: ${{STATE_INSURANCE_HOMEOWNERS_AVG}}/month
   - Recent trends in the local housing market (appreciation rates, rent increases)
     * {{STATE_NAME}} market trends: {{STATE_HOUSING_MARKET_TRENDS}}
   - Economic factors that might impact the local housing market
     * Consider {{STATE_NAME}}'s unemployment rate ({{STATE_UNEMPLOYMENT_RATE}}%) and major industries ({{STATE_MAJOR_INDUSTRIES}})
